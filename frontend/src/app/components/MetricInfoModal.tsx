...
export default function MetricInfoModal(props: MetricInfoModalProps) {
    ...
    const scrollToChart = () => {
        ...
        if (chart) {
            const targetScrollPosition = window.scrollY + chart.getBoundingClientRect().top - 0.05 * window.innerHeight;
          ...
          console.error(`Element with ID ${props.metricKey + "LineChart"} not found.`);
        }
      }
    ...
    >
        <ModalContent>
            {(onClose) =>(
                ...
                <ModalHeader>
                    {
                        props.category == 'illuminance' || props.category == 'colorTemperature'
