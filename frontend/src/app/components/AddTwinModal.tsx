import {
  ...
  interface AddTwinModalProps {
    ...
    isOpen: boolean;
    onClose: () => void;
    onAddTwin: (uid: string, currentStatus: string, predictiveStatus: {status: string, predictedTime: Date}, type: string,  cilLevel: number, location: {floor: number, section: string, area: string}) => void;
  }
  ...
  export default function AddTwinModal(props: AddTwinModalProps) {
    ...
    const onAddTwin = () => {
      props.onAddTwin(uid, currentStatus, predictiveStatus, type, cilLevel, location);
      console.log('UID:', uid);
      ...
      <>
        <Modal id="AddTwinModal" isOpen={props.isOpen} onClose={props.onClose}>
          <ModalContent>
            {(onClose) => (
              <>
                <ModalHeader className="flex flex-col gap-1">Add new digital twin</ModalHeader>
                <ModalBody>
                  <div className="flex w-full flex-wrap gap-4" style={{ marginTop: '50px' }}>
                    {/* UID */}
                    <Textarea
                    ...
                    {/* CurrentStatus */}
                    <Select isRequired labelPlacement="outside" label="Current Status" placeholder="Select current status" className="max-w-xs" value={currentStatus.toString()} onChange={(change) => {
                    ...
                    <Select 
                      ...
                      onChange={(change) => {
                          setPredictiveStatus({ ...predictiveStatus, status: change.target.value.toString() });
                      }}
                    >
                    ...
                    {/* Type */}
                    <Select isRequired labelPlacement="outside" label="Type" placeholder="Lightning type" className="max-w-xs" value={type.toString()} onChange={(change) => {
                    ...
                    <Select 
                      ...
                      onChange={(change) => {
                          setLocation({ ...location, area: change.target.value.toString() });
                      }}
                    >
                    ...
                    <Select 
                      ...
                      onChange={(change) => {
                          setLocation({ ...location, section: change.target.value.toString() });
                      }}
                    >
