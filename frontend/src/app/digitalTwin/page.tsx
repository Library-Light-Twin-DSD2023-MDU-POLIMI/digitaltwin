...
export default function DigitalTwin() {
  return (
    <div className="px-10 flex flex-col gap-4 rounded-lg w-7/9 h-full">
      ...
      <LatestValuesBar
        data={sampleData[sampleData.length - 1]}
        metadata={metricData}
        inSummaryBar={true}
      />
        ...
        <Card className="bg-primary-200">
          ...
          >
            <Select
              ...
              radius={'full'}
              key={'primary'}
              color={'default'} /* defaultSelectedKeys={["threeDays"]} */
            >
              <SelectItem key={'twentyfourHours'} value={'24 hours'}>
                {' '}
                24 hours{' '}
              </SelectItem>
              <SelectItem key={'threeDays'} value={'3 days'}>
                {' '}
                3 days{' '}
              </SelectItem>
              <SelectItem key={'oneWeek'} value={'1 week'}>
                {' '}
                1 week{' '}
              </SelectItem>
              <SelectItem key={'oneMonth'} value={'1 month'}>
                {' '}
                1 month{' '}
              </SelectItem>
              <SelectItem key={'threeMonths'} value={'3 months'}>
                {' '}
                3 months{' '}
              </SelectItem>
              <SelectItem key={'oneYear'} value={'1 year'}>
                {' '}
                1 year{' '}
              </SelectItem>
            </Select>
          ...
          {Object.entries(metricData).map(([metricKey, metaData]) => {
            ...
            const metricData = getNestedProperty(
